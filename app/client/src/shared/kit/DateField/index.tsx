...
const normalizeDateValue = (value?: string) => {
  ...
  return normalizedValue.isValid() &&
    normalizedValue.format(DATE_ONLY_FORMAT) === value
    ? value
    : ''
}
...
const DateField = ({
  ...
  const minDate = normalizedMin ? dayjs(normalizedMin) : null
  const maxDate = normalizedMax ? dayjs(normalizedMax) : null
  ...
  const displayValue = selectedDate
    ? displayFormatter.format(selectedDate.toDate())
    : helperLabel
  ...
  const dayOptions = useMemo(
    () =>
      Array.from({ length: draftDate.daysInMonth() }, (_, index) => ({
        disabled: Boolean(
          (minDate &&
            createDate(draftDate.year(), draftDate.month(), index + 1).isBefore(
              minDate,
              'day'
            )) ||
          (maxDate &&
            createDate(draftDate.year(), draftDate.month(), index + 1).isAfter(
              maxDate,
              'day'
            ))
        ),
  ...
  const monthOptions = useMemo(
    () =>
      Array.from({ length: 12 }, (_, index) => ({
        disabled: Boolean(
          (minDate &&
            createDate(draftDate.year(), index, 1)
              .endOf('month')
              .isBefore(minDate, 'day')) ||
          (maxDate &&
            createDate(draftDate.year(), index, 1).isAfter(maxDate, 'month'))
        ),
        label: capitalizeFirstLetter(
          monthFormatter
            .format(new Date(draftDate.year(), index, 1))
            .replace(/\./g, '')
        ),
        value: index
      })) satisfies WheelPickerOption<number>[],
    [draftDate, maxDate, minDate, monthFormatter]
  )
  const yearOptions = useMemo(() => {
    ...
    return Array.from(
      ...
      (_, index) => ({
        label: String(minimumYear + index),
        value: minimumYear + index
      })
    ) satisfies WheelPickerOption<number>[]
  }, [draftDate, normalizedMax, normalizedMin])
  const isTodayDisabled =
    (normalizedMin && todayDate < normalizedMin) ||
    (normalizedMax && todayDate > normalizedMax)
  ...
  const updateDraftPart = (
    ...
    setDraftDate((currentDate) =>
      buildDateFromParts(currentDate, { [part]: nextValue })
    )
  }
    ...
    <div className={styles.field}>
      ...
      <div className={styles.control}>
        <button
          ...
          className={clsx(
            styles.trigger,
            isPickerOpen && styles.triggerOpen,
        ...
        >
          <span
            className={clsx(
              styles.value,
              !selectedDate && styles.valuePlaceholder
            )}
          >
            {displayValue}
          </span>
        ...
        <div className={styles.sheetContent}>
          <div className={styles.sheetTopBar}>
            ...
            <div className={styles.sheetSummary}>
              ...
              <strong className={styles.sheetValue}>
                {pickerPreviewValue}
              </strong>
            </div>
            <button
                ...
                <div className={styles.wheelPickerSurface}>
                  <WheelPicker
                    ...
                    onValueChange={(nextMonth) =>
                      updateDraftPart('month', nextMonth)
                    }
                    visibleCount={PICKER_VISIBLE_COUNT}
                    optionItemHeight={PICKER_ITEM_HEIGHT}
                    scrollSensitivity={5}
