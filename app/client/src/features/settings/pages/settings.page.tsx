...
const SettingsPage = () => {
  ...
  const adminCopy = useMemo(() => getAdminCopy(locale), [locale])
  const { isTelegramEnvironment, setPreferredLocale, user } = useAuthSession()
  ...
  const handleTogglePreference = useCallback(
    async (
      field: 'reminderTodayEnabled' | 'reminderTomorrowEnabled',
      value: boolean
    ) => {
      setRemindersError(null)
    ...
    <Layout title={t('settings.title')} subtitle={t('settings.subtitle')}>
      <div className={styles.page}>
        <section className={styles.sectionCard}>
          ...
          {remindersHint ? (
            <p className={styles.hint}>{remindersHint}</p>
          ) : null}
          {remindersError ? (
            <p className={styles.error}>{remindersError}</p>
          ) : null}
        ...
        {user?.isAdmin ? (
          <section className={styles.sectionCard}>
            <div className={styles.sectionHeader}>
              <span className={styles.eyebrow}>
                {adminCopy.settingsEyebrow}
              </span>
              <h2 className={styles.sectionTitle}>{adminCopy.settingsTitle}</h2>
              <p className={styles.sectionDescription}>
                {adminCopy.settingsDescription}
              </p>
            </div>
