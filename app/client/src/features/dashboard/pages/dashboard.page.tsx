...
const DashboardPage = () => {
  ...
  const renderSectionSkeleton = (title: string) => (
    <section key={title} className={styles.sectionCard} aria-hidden="true">
      ...
      <div className={styles.sectionBody}>
        {Array.from({ length: DASHBOARD_SKELETON_SECTION_ITEMS }).map(
          (_, index) => (
      ...
      <div className={styles.page}>
        ...
        {summaryError ? (
          <p className={styles.error}>
            {getApiErrorMessage(summaryError, t('dashboard.errorLoad'))}
          </p>
        ) : null}
        ...
        {isSummaryLoading ? (
          <div className={styles.sectionsGrid}>
            {[
              ...
              .slice(0, DASHBOARD_SKELETON_SECTION_COUNT)
              .map(renderSectionSkeleton)}
          </div>
        ) : null}
