...
const BookingsPage = () => {
  ...
  const selectedStatusFilter = BOOKING_STATUS_VALUES.includes(
  ...
  >(() => getStoredBookingDraft())
  const draftRestoreRef = useRef<Partial<BookingFormValues> | undefined>(
    storedBookingDraftSeed
  )
  ...
  const activeAssets = useMemo(
    () => assets.filter((asset) => !asset.isArchived),
    [assets]
  )
  ...
  const hasDraftPrefill = BOOKING_PREFILL_KEYS.some((key) =>
  ...
  const bookingFilters = useMemo<BookingFilters>(
    () => ({
      ...(selectedAssetFilter !== ALL_FILTER_VALUE
  ...
  const { data: selectedBookingDetail, isLoading: isSelectedBookingLoading } =
    useBookingQuery(
      selectedBookingId,
      Boolean(selectedBookingId && !bookingFromList)
    )
  const selectedBooking = bookingFromList ?? selectedBookingDetail
  const selectedBookingAsset = useMemo(
    () => assets.find((asset) => asset.id === selectedBooking?.assetId),
    [assets, selectedBooking]
  )
  const bookingAssetOptions = useMemo(() => {
    if (!selectedBookingAsset || !selectedBookingAsset.isArchived) {
      return assetOptions
    }

    return [
      {
        label: selectedBookingAsset.name,
        value: selectedBookingAsset.id
      },
      ...assetOptions.filter(
        (option) => option.value !== selectedBookingAsset.id
      )
    ]
  }, [assetOptions, selectedBookingAsset])
  ...
  const shouldLoadPendingMetric =
    assets.length > 0 && selectedStatusFilter === ALL_FILTER_VALUE
  const { data: pendingBookingsTotal = 0, isLoading: isPendingMetricLoading } =
  ...
  const isComposerVisible =
    ...
    (Boolean(selectedBooking) ||
      (hasActiveAssets && (isComposerOpen || hasDraftPrefill)))
  ...
  useEffect(() => {
    ...
    nextSearchParams.set(
      ROUTE_QUERY_KEYS.FILTER_ASSET_ID,
  ...
  const updateAssetFilter = useCallback(
    (nextValue: string) => {
      const nextSearchParams = new URLSearchParams(searchParams)
      ...
      const nextAssetFilter =
        nextValue === ALL_FILTER_VALUE ? undefined : nextValue
  ...
  const updateStatusFilter = useCallback(
    (nextValue: BookingStatus | typeof ALL_FILTER_VALUE) => {
      const nextSearchParams = new URLSearchParams(searchParams)
    ...
    <Layout title={t('bookings.title')} subtitle={t('bookings.subtitle')}>
      <div className={styles.page}>
          ...
          <>
            <section className={styles.summaryBar}>
              ...
              <Button
                ...
                disabled={isAssetsLoading || !hasActiveAssets}
              >
            ...
            <section className={styles.listPanel}>
              ...
              <div className={styles.bookingList}>
                {isPageLoading
                  ? Array.from({ length: BOOKING_SKELETON_COUNT }).map(
                      (_, index) => <BookingCardSkeleton key={index} />
                    )
                  : bookings.map((booking) => (
                      ...
                      >
                        <BookingCard
                          booking={booking}
                          bikeName={
                            assetNameMap[booking.assetId] ??
                            t('bookings.unknownBike')
                          }
                          isDeleting={
                            deleteBooking.isPending &&
                            deleteBooking.variables === booking.id
                          }
                          isEditing={selectedBookingId === booking.id}
                          isUpdatingStatus={
                            updateBookingStatus.isPending &&
                            updateBookingStatus.variables?.bookingId ===
                              booking.id
                          }
        ...
        <ScreenSheet open={isComposerVisible} onClose={handleCloseComposer}>
          <BookingFormCard
            assetOptions={bookingAssetOptions}
