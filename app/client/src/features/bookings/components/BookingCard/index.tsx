...
const BookingCard = ({
  ...
  return (
    <article
      className={clsx(styles.card, compact && styles.compactCard, className)}
    >
      <div className={styles.header}>
        <p className={styles.kicker}>{bikeName}</p>
        <span
          className={
            styles[booking.status === 'paid' ? 'badgePaid' : 'badgePending']
          }
        >
          {booking.status === 'paid'
      ...
      <div className={styles.metaRow}>
        ...
        <div className={`${styles.metaBlock} ${styles.metaBlockDuration}`}>
          ...
          <strong className={styles.metaValue}>
            {t('booking.card.days', { count: duration })}
          </strong>
        </div>
      </div>
      ...
      <div className={styles.actions}>
        <div className={styles.leadingActions}>
          <button
            ...
            className={clsx(
              styles.iconAction,
              isEditing && styles.iconActionActive
            )}
            disabled={isDeleting}
            aria-label={
              isEditing ? t('booking.card.editing') : t('booking.card.edit')
            }
          ...
          <button
            ...
            disabled={isBusy}
            aria-label={
              isDeleting ? t('booking.card.deleting') : t('booking.card.delete')
            }
